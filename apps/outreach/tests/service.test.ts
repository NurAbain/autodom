...
beforeEach(async () => {
  delivered.length = 0;
  await pool.query(
    "TRUNCATE autodom_outreach.instagram_watch_pacing,autodom_outreach.instagram_observations,autodom_outreach.instagram_watches,autodom_outreach.contacts,autodom_outreach.deliveries,autodom_outreach.campaigns,autodom_outreach.images,public.owner_vehicles,public.events,public.listings CASCADE",
  );
  await pool.query(
...
describe("durable marketplace delivery boundary", () => {
  ...
  it("combines the property-exchange audience filter with every existing listing filter", async () => {
    await catalog.upsertListings(
      [
        makeListing({
          id: "property-match",
          title: "Toyota Camry property match",
          url: "https://mashina.kg/details/property-match",
          source: "mashina.kg",
          city: "Бишкек",
          year: 2020,
          availability: "В наличии",
          price_usd_minor: 2_000_000,
          price_kgs_minor: 170_000_000,
          catalog_attributes: { exchange_option: "Обмен на недвижимость" },
        }),
        makeListing({
          id: "ordinary-match",
          title: "Toyota Camry ordinary match",
          url: "https://mashina.kg/details/ordinary-match",
          source: "mashina.kg",
          city: "Бишкек",
          year: 2020,
          availability: "В наличии",
        ...
        makeListing({
          id: "property-wrong-city",
          title: "Toyota Camry wrong city",
          url: "https://mashina.kg/details/property-wrong-city",
          source: "mashina.kg",
          city: "Ош",
          year: 2020,
          availability: "В наличии",
        ...
        makeListing({
          id: "property-wrong-year",
          title: "Toyota Camry wrong year",
          url: "https://mashina.kg/details/property-wrong-year",
          source: "mashina.kg",
          city: "Бишкек",
          year: 2010,
          availability: "В наличии",
    ...
    const result = await service.preview({
      ...
      priceMin: 19_999,
      priceMax: 20_001,
      propertyExchangeOnly: true,
    });

    expect(result.candidates.map((candidate: Candidate) => candidate.listingId)).toEqual([
      "property-match",
    ]);
  });
