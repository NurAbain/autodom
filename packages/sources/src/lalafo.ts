...
const categoryCache = new WeakMap<
  DocumentTransport,
  { expiresAt: number; value: Promise<PassengerCategory> }
>();
...
export function parsePassengerCategory(document: string): PassengerCategory {
  const scripts = [
    ...document.matchAll(
      /<script\b[^>]*\bid\s*=\s*["']__NEXT_DATA__["'][^>]*>([\s\S]*?)<\/script\s*>/gi,
    ),
  ];
  if (scripts.length !== 1)
    throw new SourceError("Lalafo passenger bootstrap missing or ambiguous");
  const nodes = objects(parseJson(scripts[0]![1]!));
  if (
    !nodes.some(
      (node) =>
        node.id === PASSENGER_CATEGORY_ID &&
        node.url === "/avtomobili-s-probegom" &&
        node.name === "Продажа авто",
    )
  ) {
  ...
  for (const node of nodes) {
    ...
    if (
      ...
      id === 1501 ||
      id === 2000 ||
      id === PASSENGER_CATEGORY_ID
    )
      continue;
    if (
      !/^\/(?:kyrgyzstan\/)?avtomobili-s-probegom\/(?:prodazha-avtomobiley\/)?[^/?#]+\/?$/.test(
        text(node.url),
      )
    )
      continue;
  ...
  return {
    id: PASSENGER_CATEGORY_ID,
    allowedIds: new Set([PASSENGER_CATEGORY_ID, ...brands.keys()]),
    brands,
  };
}
...
function photoUrls(images: unknown): string[] {
  ...
  for (const image of images) {
    ...
    for (const key of ["original_url", "original_webp_url", "thumbnail_url"]) {
      ...
      if (!value.startsWith("https://img5.lalafo.com/i/posters/") || /[\s\\\p{Cc}]/u.test(value))
        continue;
      try {
        ...
        /* Optional malformed photos stay unknown. */
      }
    }
...
function listingUrl(value: unknown, id: number): string {
  ...
  if (!raw || /[\s\\\p{Cc}]/u.test(raw))
    throw new SourceError("Lalafo advertisement URL missing or invalid");
  let url: URL;
  try {
    url = new URL(raw, "https://lalafo.kg");
  } catch (cause) {
    throw new SourceError("Invalid Lalafo advertisement URL", { cause });
  }
  if (
    url.origin !== "https://lalafo.kg" ||
    url.username ||
    url.password ||
...
function parseListing(item: ObjectValue, id: number, categoryMake: string): Listing {
  ...
  if (!Array.isArray(attributes) || !attributes.every(isObject))
    throw new SourceError("Lalafo vehicle attributes schema changed");
  ...
  const titleModel =
    make && vehicle.toLowerCase().startsWith(`${make.toLowerCase()} `)
      ? vehicle.slice(make.length).trim()
      : "";
  const model = attrs.get("модель") ?? titleModel;
  const yearValue = attrs.get("год выпуска") ?? attrs.get("год") ?? titleParts?.[2] ?? "";
  const year =
    /^[0-9]{4}$/.test(yearValue) && Number(yearValue) >= 1800 && Number(yearValue) <= 2200
      ? Number(yearValue)
      : null;
  ...
  return makeListing({
    ...
    mileage: attrs.get("пробег (км)")
      ? `${attrs.get("пробег (км)")} km`
      : (attrs.get("пробег") ?? ""),
...
export function parsePage(document: string, page: number, category: PassengerCategory): SourcePage {
  ...
  if (!isObject(data) || !Array.isArray(data.items) || !isObject(data._meta))
    throw new SourceError("Lalafo feed schema changed");
  ...
  if (
    ...
    (size as number) < 1 ||
    (size as number) > PAGE_SIZE ||
    currentPage !== page ||
    !Number.isSafeInteger(pageCount)
  )
    throw new SourceError("Lalafo pagination schema changed");
  const pages = Math.ceil((total as number) / (size as number));
  if (pageCount !== pages && !(total === 0 && pageCount === 1))
    throw new SourceError("Lalafo page count contradicts total");
  ...
  if ((data.items.length > 0 && page > pages) || (data.items.length === 0 && page <= pages))
    throw new SourceError("Lalafo item count contradicts pagination");
  const listings = new Map<string, Listing>();
  for (const item of data.items) {
    if (!isObject(item)) throw new SourceError("Lalafo listing schema changed");
    if (item.country_id !== 12)
      throw new SourceError("Lalafo feed contains a non-Kyrgyzstan listing");
    ...
    if (!category.allowedIds.has(itemCategory))
      throw new SourceError("Lalafo feed contains a non-passenger category");
