            ...
            data: [
              {
                ...
                id: 1608,
                name: "Toyota",
                url: "/kyrgyzstan/avtomobili-s-probegom/prodazha-avtomobiley/toyota",
              },
            ],
          },
        },
...
describe("Lalafo passenger feed", () => {
  it("accepts brand categories directly beneath the official passenger route", () => {
    const gacBootstrap = bootstrap
      .replace('"id":1608', '"id":11846')
      .replace('"name":"Toyota"', '"name":"GAC"')
      .replace("/prodazha-avtomobiley/toyota", "/gac");
    const result = parseFeed(
      document([ad({ category_id: 11846, title: "GAC GS8: 2024 г., 2 л, Автомат" })]),
      1,
      parsePassengerCategory(gacBootstrap),
    );
    expect(result.listings[0]).toMatchObject({ make: "GAC", model: "GS8", year: 2024 });
  });
  it("preserves the stated currency and public identity without storing seller data or inventing stock", () => {
    const result = parsePage(
      document([
        ...
        ad({
          id: 101,
          url: "/bishkek/ads/toyota-highlander-id-101",
          price: 25900,
          currency: "KGS",
        }),
      ]),
    ...
    expect(result.listings[0]).toMatchObject({
      ...
      original_price_minor: 1350000,
      price_usd_minor: 1350000,
    ...
    expect(result.listings[1]).toMatchObject({
      ...
      original_price_minor: 2590000,
      price_usd_minor: null,
      price_kgs_minor: 2590000,
    });
  ...
  it("uses structured vehicle data while leaving unsupported title guesses and mileage unknown", () => {
    const [structured, unknown] = parsePage(
      document([
        ad({
          title: "Продам автомобиль",
          params: [
            { name: "Марка", value: "Toyota" },
            { name: "Модель", value: "Camry" },
            { name: "Год выпуска", value: "2019" },
          ],
        }),
        ad({
          id: 101,
          url: "/bishkek/ads/car-id-101",
          title: "Срочно, звоните после 2020",
          category_id: 1502,
        }),
      ]),
    ).listings;
  ...
  it("keeps unsupported or invalid prices unknown and rounds decimal tokens without floating-point drift", () => {
    for (const changes of [
      ...
      { price: true },
      { price: "NaN" },
      { price: "90071992547409.92" },
      { currency: "EUR" },
      { price_type: 999 },
    ]) {
      expect(parsePage(document([ad(changes)])).listings[0]).toMatchObject({
  ...
  it("rejects wrong categories, countries and identity URLs rather than publishing unrelated listings", () => {
    for (const changes of [
      ...
      { country_id: 1 },
      { id: null },
      { url: "https://evil.example/bishkek/ads/car-id-100" },
      { url: "/bishkek/ads/car-id-101" },
    ]) {
  ...
  it("uses API pagination despite sponsored extras and deduplicates advertisements, not cars", () => {
    const result = parsePage(
      document([ad(), ad({ price: 14000 }), ad({ id: 101, url: "/bishkek/ads/car-id-101" })], {
        totalCount: 5,
        pageCount: 5,
        perPage: 1,
      }),
    ...
    expect(() => parsePage(document([], { totalCount: 5, pageCount: 5, perPage: 1 }))).toThrow(
      SourceError,
    );
    ...
    expect(() =>
      parsePassengerCategory(
        bootstrap.replace("/prodazha-avtomobiley/toyota", "/zapchasti/toyota"),
      ),
    ).toThrow(SourceError);
